...
class dijkstra_path_pub :
    def __init__(self):
        ...
        firebase_admin.initialize_app(cred, {'projectID' : 'testpjt-653cf'})

        db = firestore.client()
        doc_ref = db.collection(u'Reservation').document(u'destination')
        doc_ret = db.collection(u'Ego').document(u'return_gps') # connect db + return gps
        doc_ret2 = db.collection(u'Reservation').document(u'return_btn') # connect db + return btn
        
        self.ret_flag = doc_ret2.get().to_dict()["return_btn"]  # get return flag
        
        # if self.ret_flag == True: 
        #     self.gps_lat = doc_ret.get().to_dict()["lat"]  
        #     self.gps_lon = doc_ret.get().to_dict()["lon"]
            
        # else: 
        #     self.gps_lat = doc_ref.get().to_dict()["des_lat"]  
        #     self.gps_lon = doc_ref.get().to_dict()["des_lon"]  
