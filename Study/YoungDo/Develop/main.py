...
def on_snapshot_choice(doc_snapshot, changes, read_time):
    for doc in doc_snapshot:
        ...
        if choice == True:
            subprocess.Popen(['gnome-terminal', '--tab', '-e', 'bash -c "roslaunch ssafy_2 test.launch"'])

def on_snapshot_reservation(doc_snapshot, changes, read_time):
    for doc in doc_snapshot:
        reservation = doc_ref_reservation.get().to_dict()["reservation_btn"]
        if reservation == True:
            subprocess.Popen(['gnome-terminal', '--tab', '-e', 'bash -c "rosrun ssafy_2 autodriving_dist_minus2.py"'])
